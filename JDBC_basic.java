...
public class Dev {
	...
	public static void main(String args[]) {
		...
		try {
			...
			myConnection = DriverManager.getConnection("jdbc:oracle:thin:@192.168.0.104:1521/ORCLPDB", "bdsm", "bdsm");
			...
			myStatement.executeUpdate("INSERT INTO customers "
					+ "(customer_id, first_name, last_name, dob, phone) VALUES (" + customerId + ", '" + firstName
					+ "', '" + lastName + "', " + "TO_DATE('" + dob + "', 'YYYY, MM, DD'), '" + phone + "')");
			...
			myStatement.executeUpdate(
					"UPDATE customers " + "SET first_name = '" + firstName + "' " + "WHERE customer_id = 1");
			...
			ResultSet customerResultSet = myStatement
					.executeQuery("SELECT customer_id, first_name, last_name, dob, phone " + "FROM customers");
			...
			ResultSet productResultSet = myStatement.executeQuery(
					"SELECT product_id, product_type_id, price " + "FROM products " + "WHERE product_id = 12");
			System.out.println("Retrieved row from products table");

			while (productResultSet.next()) {
				System.out.println("product_id = " + productResultSet.getInt("product_id"));
				System.out.println("product_type_id = " + productResultSet.getInt("product_type_id"));
				...
				// Java null value
				java.lang.Integer productTypeId = (java.lang.Integer) productResultSet.getObject("product_type_id");
			...
			myStatement.execute("CREATE TABLE addresses (" + "  address_id INTEGER CONSTRAINT addresses_pk PRIMARY KEY,"
					+ "  customer_id INTEGER CONSTRAINT addresses_fk_customers "
					+ "    REFERENCES customers(customer_id)," + "  street VARCHAR2(20) NOT NULL,"
					+ "  city VARCHAR2(20) NOT NULL," + "  state CHAR(2) NOT NULL" + ")");
